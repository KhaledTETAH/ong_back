...
from apps.organizations.models import (
	Organization, OrganizationDocument, OrganizationMember, Follow, Offer
)
...
class Command(BaseCommand):
	help = 'Seeds the database with realistic test data. Use --flush to wipe the DB first.'
	...
	def handle(self, *args, **options):
		...
		# 2. Users
		admin_user = self.create_user("admin@plateforme.org", "admin123", Role.ADMIN, Status.ACTIVE)
		ngo_owner = self.create_user("directeur@lumiere-oran.org", "password123", Role.NGO_MEMBER, Status.ACTIVE)
		ngo_recruiter = self.create_user("recruteur@lumiere-oran.org", "password123", Role.NGO_MEMBER, Status.ACTIVE)
		candidate = self.create_user("candidate@email.com", "password123", Role.CANDIDATE, Status.ACTIVE)
		referent = self.create_user("referent@entreprise.com", "password123", Role.VOLUNTEERING_REFERENT, Status.ACTIVE)

		# 3. Organizations
		org1 = self.create_organization(
			"Association Lumière d'Oran", ngo_owner, countries['DZA'], "Oran",
			OrganizationType.ASSOCIATION, VerificationStatus.VERIFIED,
			"12345", "2011", causes
		)
		org2 = self.create_organization(
			"Fondation Espoir Maghreb", ngo_owner, countries['MAR'], "Casablanca",
			OrganizationType.FOUNDATION, VerificationStatus.CERTIFIED_PLUS,
			"F-9876", "2005", causes
		)
		org3 = self.create_organization(
			"Les Amis de la Terre Afrique", ngo_owner, countries['FRA'], "Paris",
			OrganizationType.NGO, VerificationStatus.IN_PROGRESS,
			"RNA-5544", "2018", causes
		)
		...
		OrganizationMember.objects.create(organization=org1, user=ngo_owner, role=MemberRole.ADMIN)
		OrganizationMember.objects.create(organization=org1, user=ngo_recruiter, role=MemberRole.RECRUITER)
		...
		self.stdout.write(self.style.SUCCESS('Admin login: admin@plateforme.org / admin123'))
		self.stdout.write(self.style.SUCCESS('NGO login: directeur@lumiere-oran.org / password123'))
	...
	def seed_countries(self):
		...
		data = [
			("France", "FRA"), ("Algérie", "DZA"), ("Maroc", "MAR"),
			("Tunisie", "TUN"), ("Belgique", "BEL"), ("Suisse", "CHE")
		]
		countries = {}
	...
	def seed_causes(self):
		...
		data = ["Éducation", "Solidarité", "Jeunesse", "Environnement", "Santé", "Droits de l'homme"]
		causes = {}
		for name in data:
	...
	def seed_skills(self):
		...
		data = ["Gestion de projet", "Communication", "Développement web", "Collecte de fonds", "Juridique", "Pédagogie"]
		skills = {}
		for name in data:
	...
	def create_organization(self, name, owner, country, city, org_type, verification, registry, founded, causes):
		"""
		...
		org, created = Organization.objects.get_or_create(
			...
			defaults={
				...
				'is_active': True,
				'logo_url': 'https://ui-avatars.com/api/?name=' + name.replace(' ', '+') + '&background=0D9488&color=fff&size=128',
		...
		offers_data = [
			{
				...
				'city': "Oran", 'remote': RemoteMode.ON_SITE,
				'duration': "6 mois", 'status': OfferStatus.PUBLISHED,
			...
			},
			{
				...
				'type': EngagementType.VOLUNTEERING,
				'city': "Oran", 'remote': RemoteMode.ON_SITE,
				'duration': "2 mois", 'status': OfferStatus.PUBLISHED,
			...
			},
			{
				...
				'type': EngagementType.VOLUNTEERING,
				'city': "Oran", 'remote': RemoteMode.ON_SITE,
				'duration': "Récurrent", 'status': OfferStatus.PUBLISHED,
			...
			},
			{
				...
				'city': "Casablanca", 'remote': RemoteMode.HYBRID,
				'duration': "3 mois", 'status': OfferStatus.PUBLISHED,
			...
			},
			{
				...
				'city': "Paris", 'remote': RemoteMode.REMOTE,
				'duration': "1 mois", 'status': OfferStatus.DRAFT,
		...
		for data in offers_data:
			offer, created = Offer.objects.get_or_create(
				...
				defaults={
					...
					'country': countries['DZA'] if data['city'] == 'Oran' else countries['MAR'] if data['city'] == 'Casablanca' else countries['FRA'],
					'city': data['city'],
					...
					'budget': "Gratuit / Bénévolat" if data['type'] == EngagementType.VOLUNTEERING else "1500€ - 2500€",
					'published_at': timezone.now() - timedelta(days=random.randint(1, 30)) if data['status'] == OfferStatus.PUBLISHED else None,
