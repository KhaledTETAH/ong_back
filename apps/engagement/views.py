...
from .models import Country, Offer, OfferEvent, Organization, SavedOffer, SponsorSlot, SponsorshipMission
...
def published_offers():
	return Offer.objects.filter(status="published").select_related("organization", "country", "organization__country").prefetch_related(
		"causes",
		"languages",
		"skills",
...
class HomeView(APIView):
	...
	def get(self, request):
		...
		sponsor = SponsorSlot.objects.filter(placement="homepage", is_active=True).order_by("-starts_at").first()
		return SuccessResponse(
			{
				"stats": {
					"open_offers": published_offers().count(),
					"verified_organizations": Organization.objects.filter(is_active=True, verification_status__in=["verified", "certified_plus"]).count(),
					"candidate_price": "100 %",
				},
				"featured_offers": OfferSummarySerializer(featured, many=True).data,
				"coverage_countries": list(Country.objects.filter(is_covered=True).values("code", "name_fr")),
				"sponsor": {
...
class OfferListView(APIView):
	...
	def get(self, request):
		...
		if request.query_params.get("sort") == "oldest":
			...
			qs = qs.order_by("-published_at")
		return SuccessResponse(OfferSummarySerializer(qs.distinct(), many=True).data, "Offers retrieved.")
...
class OfferDetailView(APIView):
	...
	def get(self, request, slug):
		...
		return SuccessResponse(OfferDetailSerializer(offer, context={"request": request}).data, "Offer retrieved.")
...
class SimilarOffersView(APIView):
	...
	def get(self, request, slug):
		...
		offers = published_offers().exclude(pk=offer.pk).filter(Q(engagement_type=offer.engagement_type) | Q(causes__id__in=cause_ids)).distinct()[:6]
		return SuccessResponse(OfferSummarySerializer(offers, many=True).data, "Similar offers retrieved.")
...
class ApplyOfferView(APIView):
	...
	def post(self, request, slug):
		offer = get_object_or_404(Offer, slug=slug, status="published")
		serializer = ApplicationSerializer(data=request.data, context={"request": request, "offer": offer})
		serializer.is_valid(raise_exception=True)
...
class OrganizationListView(APIView):
	...
	def get(self, request):
		qs = Organization.objects.filter(is_active=True).select_related("country").prefetch_related("causes").annotate(
			open_offers_count=Count("offers", filter=Q(offers__status="published"))
		)
		if q := request.query_params.get("q"):
			qs = qs.filter(Q(name__icontains=q) | Q(description__icontains=q) | Q(mission__icontains=q) | Q(city__icontains=q))
		if country := request.query_params.get("country"):
			qs = qs.filter(country_id=country.upper())
		if cause := request.query_params.get("cause"):
			qs = qs.filter(causes__slug=cause)
		...
		if request.query_params.get("sort") == "offers":
			...
			qs = qs.order_by("name")
		return SuccessResponse(OrganizationSummarySerializer(qs.distinct(), many=True).data, "Organizations retrieved.")
...
class OrganizationDetailView(APIView):
	...
	def get(self, request, slug):
		organization = get_object_or_404(
			Organization.objects.filter(is_active=True).select_related("country").prefetch_related("causes", "offers__country", "offers__causes", "offers__languages", "offers__skills"),
			slug=slug,
		)
		organization.open_offers_count = organization.offers.filter(status="published").count()
		return SuccessResponse(OrganizationDetailSerializer(organization).data, "Organization retrieved.")
...
class SponsorshipMissionCreateView(APIView):
	...
	def post(self, request):
		...
		return SuccessResponse(data, "Sponsorship mission created.", status=status.HTTP_201_CREATED)
...
class SponsorshipMissionDetailView(APIView):
	...
	def get(self, request, tracking_uuid):
		mission = get_object_or_404(SponsorshipMission.objects.prefetch_related("causes"), tracking_uuid=tracking_uuid)
		if not mission.token_matches(request.query_params.get("token", "")):
			raise PermissionDenied("Invalid tracking token.")
		return SuccessResponse(SponsorshipMissionReadSerializer(mission).data, "Sponsorship mission retrieved.")
	...
	def post(self, request, tracking_uuid):
		...
		return SuccessResponse(SponsorshipMissionReadSerializer(mission).data, "Sponsorship mission verified.")
