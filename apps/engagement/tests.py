...
class FirstSixPagesApiTests(TestCase):
  def setUp(self):
    ...
    self.skill = Skill.objects.create(
      name="Gestion de projet", slug="gestion-de-projet"
    )
    ...
    SponsorSlot.objects.create(
      placement="homepage", sponsor_name="Partenaire ethique", is_active=True
    )

  def authenticate_candidate(self):
    response = self.client.post(
      reverse("token_obtain_pair"),
      {"email": "candidate@example.test", "password": "Password123!"},
      format="json",
    )
  ...
  def test_home_offers_and_organizations_are_public(self):
    ...
    self.assertEqual(
      self.client.get(
        reverse("offers"), {"q": "education", "country": "DZ"}
      ).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.get(reverse("offer_detail", args=[self.offer.slug])).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.get(reverse("offer_similar", args=[self.offer.slug])).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.get(
        reverse("organizations"), {"country": "DZ", "cause": "education"}
      ).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.get(
        reverse("organization_detail", args=[self.organization.slug])
      ).status_code,
      status.HTTP_200_OK,
    )
  ...
  def test_sponsorship_submission_tracking_and_verification(self):
    ...
    self.assertEqual(
      self.client.get(
        reverse("sponsorship_detail", args=[tracking_uuid]), {"token": token}
      ).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.post(
        reverse("sponsorship_verify", args=[tracking_uuid]),
        {"token": token},
        format="json",
      ).status_code,
      status.HTTP_200_OK,
  ...
  def test_candidate_can_save_apply_and_share_offer(self):
    ...
    self.assertEqual(
      self.client.put(reverse("offer_saved", args=[self.offer.slug])).status_code,
      status.HTTP_200_OK,
    )
    self.assertEqual(
      self.client.post(
        reverse("offer_apply", args=[self.offer.slug]),
        {"cover_letter": "Je souhaite contribuer."},
        format="json",
      ).status_code,
    ...
    self.assertEqual(
      self.client.post(
        reverse("offer_share", args=[self.offer.slug]),
        {"channel": "copy_link"},
        format="json",
      ).status_code,
      status.HTTP_200_OK,
    ...
    self.assertEqual(
      self.client.delete(reverse("offer_saved", args=[self.offer.slug])).status_code,
      status.HTTP_200_OK,
    )
