...
class OfferDetailSerializer(OfferSummarySerializer):
  ...
  def get_is_saved(self, obj):
    ...
    return bool(
      user.is_authenticated and SavedOffer.objects.filter(user=user, offer=obj).exists()
    )
  ...
  def get_has_applied(self, obj):
    ...
    return bool(
      ...
      and Application.objects.filter(candidate=user, offer=obj).exists()
    )
...
class ShareOfferSerializer(serializers.Serializer):
  channel = serializers.ChoiceField(
    choices=["copy_link", "email", "linkedin", "facebook", "whatsapp", "other"]
  )


class SponsorshipMissionSerializer(serializers.ModelSerializer):
  country_code = serializers.CharField(
    write_only=True, required=False, allow_blank=True, max_length=2
  )
