...
class Organization(TimeStampedModel):
	...
	verification_status = models.CharField(max_length=40, choices=VERIFICATION_CHOICES, default="pending")
	is_active = models.BooleanField(default=True)
...
class Offer(TimeStampedModel):
	...
	def is_published(self):
		return self.status == "published" and (self.expires_at is None or self.expires_at > timezone.now())
...
class SavedOffer(TimeStampedModel):
	...
	user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="saved_offers")
...
class SponsorshipMission(TimeStampedModel):
	...
	status = models.CharField(max_length=40, choices=STATUS_CHOICES, default="pending_email_verification")
	verification_token_hash = models.CharField(max_length=64)
	verified_email_at = models.DateTimeField(null=True, blank=True)
	causes = models.ManyToManyField(Cause, related_name="sponsorship_missions", blank=True)
	...
	def token_matches(self, token):
		return self.verification_token_hash == hashlib.sha256((token or "").encode("utf-8")).hexdigest()
