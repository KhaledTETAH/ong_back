...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    countries = {
      "DZ": Country.objects.update_or_create(
        code="DZ", defaults={"name_fr": "Algerie", "is_covered": True, "sort_order": 1}
      )[0],
      "FR": Country.objects.update_or_create(
        code="FR", defaults={"name_fr": "France", "is_covered": True, "sort_order": 2}
      )[0],
      "MA": Country.objects.update_or_create(
        code="MA", defaults={"name_fr": "Maroc", "is_covered": True, "sort_order": 3}
      )[0],
    }
    causes = {
      "education": Cause.objects.update_or_create(
        slug="education", defaults={"name": "Education"}
      )[0],
      "inclusion-numerique": Cause.objects.update_or_create(
        slug="inclusion-numerique", defaults={"name": "Inclusion numerique"}
      )[0],
      "jeunesse": Cause.objects.update_or_create(
        slug="jeunesse", defaults={"name": "Jeunesse"}
      )[0],
    }
    languages = {
      "fr": Language.objects.update_or_create(
        code="fr", defaults={"name_fr": "Francais"}
      )[0],
      "ar": Language.objects.update_or_create(code="ar", defaults={"name_fr": "Arabe"})[
        0
      ],
    }
    skills = {
      "gestion-de-projet": Skill.objects.update_or_create(
        slug="gestion-de-projet", defaults={"name": "Gestion de projet"}
      )[0],
      "developpement-web": Skill.objects.update_or_create(
        slug="developpement-web", defaults={"name": "Developpement web"}
      )[0],
    }
    ...
    org.causes.set(
      [causes["education"], causes["jeunesse"], causes["inclusion-numerique"]]
    )
    ...
    SponsorSlot.objects.update_or_create(
      ...
      defaults={
        ...
        "label": "Partenariat",
        "copy": (
          "Emplacement reserve a un partenaire aligne avec les valeurs "
          "de la plateforme."
        ),
        "is_active": True,
      },
