...
class OrganizationMember(models.Model):
  ...
  role = models.CharField(
    max_length=40, choices=MemberRole.choices, default=MemberRole.READER
  )
...
class Follow(models.Model):
  ...
  organization = models.ForeignKey(
    Organization, on_delete=models.CASCADE, related_name="followers"
  )
...
class Offer(models.Model):
  ...
  employment_contract = models.CharField(
    max_length=40, choices=ContractType.choices, blank=True
  )
  experience_level = models.CharField(
    max_length=40, choices=ExperienceLevel.choices, blank=True
  )

  country = models.ForeignKey(Country, on_delete=models.PROTECT, related_name="offers")
  city = models.CharField(max_length=120, blank=True)
  region = models.CharField(max_length=120, blank=True)
  remote_mode = models.CharField(
    max_length=40, choices=RemoteMode.choices, default=RemoteMode.ON_SITE
  )
  ...
  status = models.CharField(
    max_length=40, choices=OfferStatus.choices, default=OfferStatus.DRAFT
  )

  visibility = models.CharField(
    max_length=20, choices=OfferVisibility.choices, default=OfferVisibility.OPEN
  )
  ...
  def is_published(self):
    return self.status == OfferStatus.PUBLISHED and (
